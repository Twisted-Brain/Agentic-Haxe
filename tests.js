...
var TestConversation = function() {
	massive_munit_TestSuite.call(this);
};
TestConversation.__name__ = "TestConversation";
TestConversation.__super__ = massive_munit_TestSuite;
TestConversation.prototype = $extend(massive_munit_TestSuite.prototype,{
	testInitialState: function() {
		var conversation = new domain_core_Conversation("test_id","test_title","test_model");
		massive_munit_Assert.areEqual(0,conversation.messages.length,null,{ fileName : "tests/TestConversation.hx", lineNumber : 15, className : "TestConversation", methodName : "testInitialState"});
	}
	,__class__: TestConversation
});
...
TestMain.main = function() {
	...
	runner.run([TestConversation,tests_e2e_E2ETest]);
};
...
var domain_core_Conversation = function(id,title,modelId,messages) {
	this.id = id;
	this.title = title;
...
domain_core_Conversation.__name__ = "domain.core.Conversation";
domain_core_Conversation.fromJson = function(data) {
	var conversation = new domain_core_Conversation(data.id,data.title,data.modelId);
	if(data.messages != null) {
		var _g = [];
		var _g1 = 0;
		var _g2 = js_Boot.__cast(data.messages , Array);
		while(_g1 < _g2.length) {
			var msgData = _g2[_g1];
			++_g1;
			_g.push(domain_core_ConversationMessage.fromJson(msgData));
		}
		conversation.messages = _g;
	}
	...
	if(data.metadata != null) {
		var _g = 0;
		var _g1 = Reflect.fields(data.metadata);
		while(_g < _g1.length) {
			var field = _g1[_g];
			++_g;
			conversation.metadata.h[field] = Reflect.field(data.metadata,field);
		}
	}
	return conversation;
};
domain_core_Conversation.prototype = {
	id: null
	,title: null
	,modelId: null
	,messages: null
	...
	,isActive: null
	,metadata: null
	,addMessage: function(role,content,timestamp) {
		var message = new domain_core_ConversationMessage(this.generateMessageId(),role,content,timestamp != null ? timestamp : new Date().getTime());
		this.messages.push(message);
		this.updatedAt = new Date().getTime();
		return message;
	}
	,addUserMessage: function(content) {
		return this.addMessage(domain_core_MessageRole.User,content);
	}
	,addAssistantMessage: function(content) {
		return this.addMessage(domain_core_MessageRole.Assistant,content);
	}
	,addSystemMessage: function(content) {
		return this.addMessage(domain_core_MessageRole.System,content);
	}
	...
	,setMetadata: function(key,value) {
		this.metadata.h[key] = value;
		this.updatedAt = new Date().getTime();
	}
	,getMetadata: function(key) {
		return this.metadata.h[key];
	}
	,archive: function() {
		this.isActive = false;
		this.updatedAt = new Date().getTime();
	...
	,generateMessageId: function() {
		return "msg_" + Std.string(new Date().getTime()) + "_" + Std.string(Math.floor(Math.random() * 1000));
	}
	,toJson: function() {
		var tmp = this.id;
		var tmp1 = this.title;
		var tmp2 = this.modelId;
		...
		var tmp3 = result;
		var tmp4 = this.createdAt;
		var tmp5 = this.updatedAt;
		var tmp6 = this.isActive;
		...
		while(key_current < key_length) {
			...
			_g.h[key] = this.metadata.h[key];
		}
		return { id : tmp, title : tmp1, modelId : tmp2, messages : tmp3, createdAt : tmp4, updatedAt : tmp5, isActive : tmp6, metadata : _g};
	}
	,__class__: domain_core_Conversation
};
var domain_core_MessageRole = $hxEnums["domain.core.MessageRole"] = { __ename__:true,__constructs__:null
	,System: {_hx_name:"System",_hx_index:0,__enum__:"domain.core.MessageRole",toString:$estr}
	,User: {_hx_name:"User",_hx_index:1,__enum__:"domain.core.MessageRole",toString:$estr}
	,Assistant: {_hx_name:"Assistant",_hx_index:2,__enum__:"domain.core.MessageRole",toString:$estr}
};
domain_core_MessageRole.__constructs__ = [domain_core_MessageRole.System,domain_core_MessageRole.User,domain_core_MessageRole.Assistant];
var domain_core_ConversationMessage = function(id,role,content,timestamp) {
	this.id = id;
	this.role = role;
...
domain_core_ConversationMessage.__name__ = "domain.core.ConversationMessage";
domain_core_ConversationMessage.fromJson = function(data) {
	var role;
	switch(data.role) {
	case "assistant":
		role = domain_core_MessageRole.Assistant;
		break;
	case "system":
		role = domain_core_MessageRole.System;
		break;
	case "user":
		role = domain_core_MessageRole.User;
		break;
	default:
		role = domain_core_MessageRole.User;
	}
	var message = new domain_core_ConversationMessage(data.id,role,data.content,data.timestamp);
	if(data.metadata != null) {
		var _g = 0;
		var _g1 = Reflect.fields(data.metadata);
		while(_g < _g1.length) {
			var field = _g1[_g];
			++_g;
			message.metadata.h[field] = Reflect.field(data.metadata,field);
		}
	}
	return message;
};
domain_core_ConversationMessage.prototype = {
	id: null
	,role: null
	,content: null
	,timestamp: null
	...
	,toJson: function() {
		...
		var tmp2 = this.content;
		var tmp3 = this.timestamp;
		var _g = new haxe_ds_StringMap();
		var h = this.metadata.h;
		var key_h = h;
		var key_keys = Object.keys(h);
		...
		return { id : tmp, role : tmp1, content : tmp2, timestamp : tmp3, metadata : _g};
	}
	,__class__: domain_core_ConversationMessage
};
...
var tests_e2e_E2ETest = function() {
	massive_munit_TestSuite.call(this);
};
tests_e2e_E2ETest.__name__ = "tests.e2e.E2ETest";
tests_e2e_E2ETest.__super__ = massive_munit_TestSuite;
tests_e2e_E2ETest.prototype = $extend(massive_munit_TestSuite.prototype,{
	setupServer: function() {
		var serverPath = haxe_io_Path.join([haxe_io_Path.directory(process.argv[0]),"..","bin","backend_server"]);
		tests_e2e_E2ETest.serverProcess = new sys.io.Process(serverPath,[]);
		haxe_Timer.delay(function() {
		},3000);
	}
	,teardownServer: function() {
		if(tests_e2e_E2ETest.serverProcess != null) {
			tests_e2e_E2ETest.serverProcess.kill();
		}
	}
	,testHomePageLoads: function(done) {
		tests_e2e_Puppeteer.launch().then(function(browser) {
			browser.newPage().then(function(page) {
				page.goto("http://localhost:8080").then(function(_) {
					page.content().then(function(content) {
						massive_munit_Assert.isTrue(content.indexOf("Haxe AI Chat") > -1,null,{ fileName : "tests/e2e/E2ETest.hx", lineNumber : 38, className : "tests.e2e.E2ETest", methodName : "testHomePageLoads"});
	...
	,__class__: tests_e2e_E2ETest
});
var tests_e2e_Puppeteer = require("puppeteer");
...
TestConversation.__meta__ = { fields : { testInitialState : { Test : null}}};
massive_munit_Assert.assertionCount = 0;
...
tests_e2e_E2ETest.__meta__ = { fields : { setupServer : { BeforeClass : null}, teardownServer : { AfterClass : null}, testHomePageLoads : { Test : null}}};
TestMain.main();
